...
def is_table_block(block: Dict[str, Any]) -> bool:
    return "tbl" in str(block.get("block_id") or "")
...
def build_chunks(
    ...
    *,
    ...
    overlap_tokens: int,
    min_chunks: int = 1,
    table_chunk_tokens: int | None = None,
    ...
    Prose and tables get different budgets and never share a chunk. They are read
    differently - prose line by line, tables by comparing rows - so mixing them
    would force one size to fit both and hand a reviewer two jobs at once.
    """
    text_budget = int(chunk_tokens)
    table_budget = int(table_chunk_tokens or chunk_tokens)

    if min_chunks and min_chunks > 1:
        total = sum(block_token_cost(block) for block in blocks)
        if total > 0:
            cap = max(4000, total // min_chunks)
            text_budget, table_budget = min(text_budget, cap), min(table_budget, cap)
    ...
    for block in blocks:
        ...
        table = is_table_block(block)
        budget = table_budget if table else text_budget
        if current and (current_is_table != table or current_tokens + cost > budget):
            chunks.append(current)
            current, current_tokens = [], 0
        current.append(block)
        current_tokens += cost
        current_is_table = table
    if current:
        chunks.append(current)
